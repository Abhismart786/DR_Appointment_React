...
// import React, { useState, useEffect } from 'react';
// import { useNavigate } from 'react-router-dom';
// import { auth, database, ref, onValue ,set ,remove} from './Firebase';
// import './DoctorHome.css';
// function DoctorHome() {
//   const navigate = useNavigate();
...
//           const doctorAppointments = Object.entries(data)
//             .filter(([key, appointment]) => {
//               // Ensure the doctor and user.displayName are not undefined or null
//               const doctorName = appointment?.doctor ? String(appointment.doctor).toLowerCase().trim() : '';
//               const userName = user.displayName ? String(user.displayName).toLowerCase().trim() : '';
//               console.log('Appointment:', appointment); // Debugging: log individual appointments
//               return doctorName === userName;  // Compare normalized names
//             })
//             .map(([key, appointment]) => ({ id: key, ...appointment }));
    ...
    if (user) {
      ...
      onValue(appointmentsRef, (snapshot) => {
        ...
        if (data) {
          // Filter appointments for the current doctor
          const doctorAppointments = Object.entries(data)
            .filter(([key, appointment]) => appointment?.doctor === user.displayName)
            .map(([key, appointment]) => ({ id: key, ...appointment }));
  ...
  const handleAcceptAppointment = (appointmentId) => {
    ...
    set(appointmentRef, { ...appointment, status: 'accepted' })
      .then(() => {
        ...
        // Send notification to user by updating appointment status in user data
        const userRef = ref(database, `users/${appointment.patientId}`);
        set(userRef, {
          ...appointment,
          appointmentStatus: 'accepted', // Notification field
        });
